...
function moveCol(columns, source, dest) {
...
function newGroupDiffCol(columns, source, combine) {
  ...
  const destIndex = destItems.findIndex((item) => {return item.id === combine.draggableId});
...
function newGroupSameCol(columns, source, combine) {
  ...
  const destIndex = sourceItems.findIndex((item) => {return item.id === combine.draggableId});
...
export function onDragEnd (columns, result) {
  ...
  if (!!result.destination) {
    ...
    if (source.droppableId === dest.droppableId && source.index === dest.index) return newColumns;

    /* Group logic */
    if (source.droppableId.includes('group') || dest.droppableId.includes('group')) {
      if (source.droppableId === dest.droppableId) {
        console.log('Same Group');
        newColumns = sameGroup(columns, source, dest);
      } else if (source.droppableId.includes('group') && !dest.droppableId.includes('group')) {
